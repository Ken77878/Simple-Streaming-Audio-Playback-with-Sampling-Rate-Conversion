						  ...
						  /* no reason to complain; already complained above */
							...
							**pcm is a multichannel float vector.  In stereo, for
							...
							if ((samples = vorbis_synthesis_pcmout(&vorbisDspState, &pcm)) > 0) {
								int clipflag = 0;
								oggSamplesArray[oggSamplesArray.size() - 1].second.reserve(oggSamplesArray[oggSamplesArray.size() - 1].second.size() + samples * vorbisInfo.channels);
								...
								for (int i = 0; i < samples; ++i)
									...
									//	printf("val: %d\n", val);
										...
										oggSamplesArray[oggSamplesArray.size() - 1].second.push_back(val);
									}
								if (clipflag)
